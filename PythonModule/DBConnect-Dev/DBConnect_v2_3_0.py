...
class DBConnect:
    ...
    class FileReader:
        """
        Read supported file formats into Pandas or GeoPandas DataFrames.
        ...
        def read_file(self, file_path: str, file_sheetname: str = None, file_layer: str = None):
            """
            Read various geospatial and tabular file types into a DataFrame.
            
            Args:
                file_path (str): Full path to the file.
                file_sheetname (str, optional): Sheet name for Excel files.
                file_layer (str, optional): Layer name for GeoPackage or zipped FileGDB.

            Special Note (for geo database files, below is the preferred structure):
                file.gdb.zip
                └── file.gdb/
                    ├── a00000001.gdbtable
                    ├── a00000001.gdbtablx
            ...
            Raises:
                ValueError: If file type is unsupported or required info (sheet/layer) is missing.
            """
            ...
            match ext:
                ...
                case ".zip":
                    if file.endswith(".gdb.zip"):
                        # Handle zipped File Geodatabase
                        gdb_name = file.replace(".zip", "")
                        vfs_path = f"/vsizip/{file_path}/{gdb_name}"

                        layers = fiona.listlayers(vfs_path)
                        if not layers:
                            raise ValueError(f"No layers found in zipped GDB: {file}")
                        
                        # Use passed layer or default to first
                        layer_to_read = file_layer if file_layer else layers[0]
                        if layer_to_read not in layers:
                            raise ValueError(f"Layer '{layer_to_read}' not found in {file}. Available: {layers}")
